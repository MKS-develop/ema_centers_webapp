...
function Treatments() {
    ...
    const TreatmentComponent = ({data, index}) => {
        return(
    ...
    return (
        <div className='flex flex-wrap justify-start items-center content-start w-full h-fit gap-3'>
            {listOfTreatments.length > 0 ? listOfTreatments.map((date, i)=> <TreatmentComponent data={date} index={i} key={i}/>) : 
            <div className='w-full h-full flex flex-col justify-center items-center text-center px-[15%]'>
                ...
                <p className='font-light text-sm text-slate-400'>Este paciente no tiene tratamientos previos</p>
            </div>
            }
        </div>
    )
