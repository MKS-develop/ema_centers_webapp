...
function History() {
    ...
    const DateComponent = ({data, index}) => {
        return(
        <div className="relative w-full flex justify-center items-center h-fit">
            ...
            <span className={`h-[40%] w-[4px] ${index === 0 ? "bg-white" : "bg-secondary" }`}></span>
            <span className='h-[20px] w-[20px] rounded-full border-4 border-secondary bg-white'></span>
            <span className={`h-[40%] w-[4px] ${index === (listOfDates.length - 1) ? "bg-white" : "bg-secondary" }`}></span>
            </div>
            <div onClick={()=>{
            ...
            <div className="flex flex-col justify-start items-start w-[30%]">
                <p className='font-medium text-[1.2rem] text-primary'>{data["date"] ?? "03/01/2023" }</p>
            ...
            <div className="flex flex-col justify-start items-start w-[30%]">
                <p className='font-light text-[0.8rem] text-slate-400'>Servicio atención</p>
    ...
    return (
        <>
            {listOfDates.length > 0 ? listOfDates.map((date, i)=> <DateComponent data={date} index={i} key={i}/>) : 
